...
namespace AgentExample.SharedServices.Services
{
    public class SkAgentRunnerService(IConfiguration configuration)
    {
        ...
        public async IAsyncEnumerable<string> ChatStream(string input, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            var plugins = await GetTaxHelperPlugins();
        ...
        {
            return Plugins.Count > 0 ? Plugins : await GetTaxHelperPlugins();
        }
       
        public static async Task<string> InvokeSummarizeFunction(string input, string functionName, string model = "gpt-3.5-turbo")
        {
            var kernel = Kernel.CreateBuilder().AddOpenAIChatCompletion(model, TestConfiguration.OpenAI.ApiKey).Build();
            var plugin = kernel.ImportPluginFromPromptDirectory(Path.Combine(PluginPath, "SummarizePlugin"), "Summarizer");
            var functionResult = await kernel.InvokeAsync(plugin[functionName], new KernelArguments { ["input"] = input });
            return functionResult.GetValue<string>() ?? "";
        }
       
        private static async Task<List<KernelPlugin>> GetTaxHelperPlugins()
        {
        ...
        private Kernel CreateKernel()
        {
            ...
            // For using OpenAI
            kernelBuilder.AddOpenAIChatCompletion(configuration["OpenAI:ChatModelId"], configuration["OpenAI:ApiKey"]);
